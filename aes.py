...
def expand_key(key):
    ...
    for i in range(4, 44):
        ...
        if i % 4 == 0:
            ...
            rcon_byte = RCON[i // 4]
            temp = bytes([temp[0] ^ rcon_byte]) + temp[1:]
...
def mix_columns(state):
    ...
    for c in range(4):
        new_col = [
            ...
            state[0][c] ^ xtime(state[1][c]) ^ mul3(state[2][c]) ^ state[3][c],
            state[0][c] ^ state[1][c] ^ xtime(state[2][c]) ^ mul3(state[3][c]),
            mul3(state[0][c]) ^ state[1][c] ^ state[2][c] ^ xtime(state[3][c])
        ]
        for r in range(4):
            state[r][c] = new_col[r]
